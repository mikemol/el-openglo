...
wiring the REAL Plymouth callbacks (fetched, session 49):
  ...
  - SetDisplayPasswordFunction(prompt, bullets): BACKLIGHT ON (attention mode) +
    a bullet per typed char — the Indiglo button press when boot blocks on YOU.
...
def dot_theme(variant):
    ...
    return (
        "[Plymouth Theme]\n"
        f"Name=EL Indiglo ({variant})\n"
        f"Description=Electroluminescent watch boot splash — {variant}\n"
        "ModuleName=script\n\n"
        "[script]\n"
        f"ImageDir=/usr/share/plymouth/themes/el-indiglo-{variant}\n"
        f"ScriptFile=/usr/share/plymouth/themes/el-indiglo-{variant}/el-indiglo.script\n"
    )
...
def script(variant):
    ...
    return f'''# EL Indiglo Plymouth theme ({variant}) — reactive phosphor watch.
...
# The Indiglo button: when boot waits for YOU, light the field + a bullet/char.
...
def render_all(variants, dir_map):
    ...
    for v in variants:
        ...
        open(os.path.join(d, "el-indiglo.plymouth"), "w").write(dot_theme(v))
        open(os.path.join(d, "el-indiglo.script"), "w").write(script(v))
        written[v] = files + [os.path.join(d, "el-indiglo.plymouth"),
                              os.path.join(d, "el-indiglo.script")]
    return written
