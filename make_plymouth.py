...
# NOBODY KNOWS AT BUILD TIME. The script plugin learns Window.GetHeight() only
# when it runs, and it can Image.Scale(width, height) (script-lib-image.c,
# plymouth 24.004 — read from the Debian source mirror, since the freedesktop
# wiki refuses fetches). A PNG rendered at U=48 and stretched to a 4K screen
# is what "raster" meant here. So: render ONCE at a high U — the raster
# analogue of vector, oversampled — and let the script scale DOWN to a digit
# height derived from the screen, laying the digits out at the module pitch
# every other surface uses (segment_topology.metrics). Downscaling is what
# Image.Scale does well; upscaling from 48 is what softened the edges.
ASSET_U = 192           # 4x the old 48: a 2K screen's 0.2·sh digit is ~430px tall = U 108
DIGIT_FRACTION = 0.20   # digit height as a fraction of screen height, at boot


def render_assets(variant, out_dir, U=ASSET_U):
...
def script(variant):
    ...
    # the module pitch as a ratio of digit height (segment_topology, four
    # datasheets, W22); the digit is 4U of the 5U image, so the image height
    # relates to H by 5/4
    pitch_ratio = ST.metrics(1.0)["pitch"]
...
# --- scale to THIS screen (⊕PLYMOUTH-VECTOR) --------------------------------
# The assets are oversampled (U={ASSET_U}); the digit is 4/5 of the image height.
# Scale every image so the digit stands {DIGIT_FRACTION} of the screen height.
target_ih = Math.Int (sh * {DIGIT_FRACTION} * 5 / 4);
f = target_ih / digit_image[0].GetHeight ();
for (i = 0; i < 10; i++)
    digit_image[i] = digit_image[i].Scale (Math.Int (digit_image[i].GetWidth () * f), target_ih);
blank_image = blank_image.Scale (Math.Int (blank_image.GetWidth () * f), target_ih);
colon_on = colon_on.Scale (Math.Int (colon_on.GetWidth () * f), target_ih);
colon_off = colon_off.Scale (Math.Int (colon_off.GetWidth () * f), target_ih);

dw = digit_image[0].GetWidth ();
dh = digit_image[0].GetHeight ();
digit_h = dh * 4 / 5;                           # the lit digit inside its padded image
pitch = Math.Int (digit_h * {pitch_ratio:.4f});   # module pitch: segment_topology.metrics
# stopwatch layout: M : S S — four slots at the module pitch, the colon in
# the ordinary gap after slot 0 (the 88:88 module keeps its pitch across it)
group_w = pitch * 3 + dw;
...
for (i = 0; i < 4; i++) {{
    ...
    slot[i].sprite.SetX (base_x + i * pitch);
    slot[i].sprite.SetY (base_y);
    slot[i].sprite.SetZ (10);
    slot[i].value = -1;
}}
colon.sprite = Sprite (colon_on);
colon.sprite.SetX (base_x + dw / 2 + pitch / 2 - colon_on.GetWidth () / 2);
